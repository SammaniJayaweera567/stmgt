...
$max_marks = '';
$due_date = '';
$pass_mark_percentage = ''; // Initialized for the form
$status = 'Published'; // Default status for new assignments
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($messages)) {
        ...
        $sql = "INSERT INTO assessments (title, description, assessment_type, class_id, academic_year_id, subject_id, teacher_id, max_marks, due_date, status, pass_mark_percentage) 
                VALUES ('$title', '$description', 'Assignment', '$class_id', '$academic_year_id', '$subject_id', '$teacher_id', '$max_marks', '$due_date', '$status', '$pass_mark_percentage')";
        
        if ($db->query($sql)) {
            header("Location: manage_assignments.php?status=added");
            exit();
        } else {
            $messages['main'] = "Database error: Could not create the assignment. " . $db->error;
        }
    }
}

// Fetch data for dropdowns (same as Exam module add_exam.php)
$academic_years = $db->query("SELECT * FROM academic_years WHERE status='Active'");
// Fetch classes combining class_levels, subjects, class_types for full name display
